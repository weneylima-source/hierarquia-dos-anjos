...
{ num:5, name:"Virtudes", img:"virtudes",
  ...
  apoc:{
    ...
    angels:[
      {name:"Gabriel",title:"também associado ao coro das virtudes",specialNote:true,source:"Apócrifo — 1 Enoque",refs:[{cite:"1 Enoque 40:9",quote:"Gabriel como um dos quatro anjos principais diante do Senhor."}]},
...
{ num:7, name:"Principados", img:"principados",
  ...
  canon:{
    ...
    angels:[{name:"Miguel",title:"príncipe de Israel",specialNote:true,source:"Cânon — Daniel",refs:[{cite:"Daniel 10:13,21; 12:1",quote:"Naquele tempo se levantará Miguel, o grande príncipe que se levanta a favor dos filhos do teu povo."}]}],
    readings:[
  ...
  apoc:{
    ...
    angels:[
      {name:"Miguel",title:"príncipe de Israel",specialNote:true,source:"Apócrifo — 1 Enoque",refs:[{cite:"1 Enoque 20:5",quote:"Miguel, um dos santos anjos, posto sobre a melhor parte da humanidade."}]},
...
function chipHtml(a,id){
  const note = a.specialNote ? ' <span class="note-link" data-note-target="orders-note">Ver nota especial</span>' : '';
  return '<button type="button" class="angel-chip" data-angel="'+id+'"><span class="nm">'+a.name+'</span><span class="role">'+a.title+note+'</span></button>';
}
...
function openModal(id){
  ...
  const noteLink = angel.specialNote ? ' <a href="#orders-note" class="note-link" data-note-target="orders-note">Ver nota especial</a>' : '';
  modalFunction.innerHTML = (angel.title.charAt(0).toUpperCase()+angel.title.slice(1)+". "+choir.fn).replace(/&/g,"&amp;").replace(/</g,"&lt;")+noteLink;
...
document.body.addEventListener("click", function(e){
  ...
  if(noteLink){
    e.preventDefault();
    closeModal();
    const target = document.getElementById(noteLink.dataset.noteTarget || "orders-note");
